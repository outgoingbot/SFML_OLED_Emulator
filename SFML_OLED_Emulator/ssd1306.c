...
//	SSD1306_WRITECOMMAND(0x0F); //0xFF izzle (CONTRAST VALUE - HIGHER is BRIGHTER)
...
//	SSD1306_WRITECOMMAND(0x80); //--set divide ratio //0xF0 izzle
//	SSD1306_WRITECOMMAND(0xD9); //--set pre-charge period
//	SSD1306_WRITECOMMAND(0x22); //22 - zzile
...
//	SSD1306_WRITECOMMAND(0x00); //0x20,0.77xVcc //izzle
...
//ToDo: need to decide if I like the way the I2c1 and DMA ready checks are being done
//isnt the callback tranfer complete enough?
...
//so im fulling up page buffers on the SSD1306 that are not visible. But, the code is much simpler/faster now.
	...
	//if(hdma_i2c1_tx.State == HAL_DMA_STATE_READY){ //these could be if statements if UpdateScreen was called in a timer.
	//	if(hi2c1.State == HAL_I2C_STATE_READY){ //dma check to transmit. should be a if statement to prevent blocking
	//		if(txComplete){
	//			txComplete = 0;
